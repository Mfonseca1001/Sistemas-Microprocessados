;Relógio digital com timer em assembly............. Jhonathas e Marcele
;Dia 20/09/2016
...
inicio:   jmp       inicio2
          ...
          cseg      at 000bh         ; interrupção timer 0
...
continua: 
          ...
          setb      TR0
          setb      TR1
          mov       ie,#8Ah              ; habilita a interrupção timer 0 e 1
conversao: 
	SETB      DSP0
          mov       seg,#0
          mov       min,#0
...
mostrad_sem:; DIA DA SEMANA IMPORTANTE!!!
		  ...
		  mov		b,#4	;
		  div		ab
	            mov		r1,a	; valor de B= ano não bissexto	
	            mov     	a,ano
                      mov    	          b,#4
  	            div     	ab 
                      mov     	a,b 
	            jz		tstmes 	;ano bissexto? se sim, vai para teste do mês, se não, segue normal
	            jmp		passo3
